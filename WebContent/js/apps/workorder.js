...
function checkRadio(boolVal, idTrue, idFalse) {
    if (boolVal) {
        $(idTrue).prop('checked', true);
    } else {
        $(idFalse).prop('checked', true);
    }
}
...
function generateWorkId() {
    $.ajax({
        ...
        $('#workorderId').val(data[0].maxid);
        getPaperList();
    });
}

function onEditWorkorder() {
    if (workList !== null) {
        $.ajax({
            url: '/report-build/apis/workorder',
            data: {
...
function listWorkorder() {
    var obj = '';
    $.each(workList, function (index, val) {
    ...
    $('#btn-footer-set').html('<button type="button" onclick="selectWorkorderForEdit($(\'#customer_id\').val())" class="btn btn-primary" data-dismiss="modal">เลือก</button>'
    +'<button type="button" class="btn btn-secondary" data-dismiss="modal">ยกเลิก</button>');
    $('#listCustomer').modal();
}

function selectWorkorderForEdit(workorderId) {
    $.ajax({
        url: '/report-build/apis/workorder',
        data: {
            order: 'getwork',
        ...
        $.each(data, function (index, val) {
            $('#workorderId').val(val.workorderId);
            $('#quotationId').val(val.quotation.quotationId);
            $('#saleNo').val(val.sale.saleId);
            $('#dispatch').val(val.workDispatch);

            $('#customerId').val(val.customer.customerId);
            $('#customerName').val(val.customer.customerOrgName);
            $('#customerAddr').val(val.customer.customerAddr);
            $('#customerContact').val(val.customer.customerContact);

            $('#productId').val(val.product.productId);
            $('#productName').val(val.product.productName);
            $('#productQuantity').val(val.product.productQuantity);
            $('#productTpage').val(val.product.productNOofPart);
            $('#productConstat').prop('checked', val.product.productConstat==='1'?true:false);
            $('#productUnitSet').prop('checked', val.product.productUnitSet==='1'?true:false);
            $('#productOthers').val(val.product.productOther);
            $('#productHeight').val(val.product.productHeight);
            $('#productWidth').val(val.product.productWidth);
            $('#productStart').val(val.product.productNOstart);
            $('#productEnd').val(val.product.productNOend);
            
            checkRadio(val.workSprocketHole, '#haveSprocetHole', '#notHaveSprocetHole');
            checkRadio(val.workCrimping, '#haveCrim', '#notHaveCrim');
            checkRadio(val.workGlue, '#haveGlue', '#notHaveGlue');
            $('#formFolding').val(val.workFormFolding);
            $('#packingSet').val(val.workPackingSet);
            $('#packingBox').val(val.workPackingBox);
            $('#workAddition').val(val.workAdditional);

            setPartTable(val.product.productNOofPart);
            getProductSpecForEdit(val.product.productId);
        });
        onDupicate(true);
    });
}
...
function listQuotation() {
    ...
    $.each(quotationList, function (index, val) {
        obj += '<option value="'+val.quotationId+'">รหัสใบเสนอราคา '+val.quotationId+' -- ชื่อผลิตภัณฑ์ '+val.product.productName+'</option>'
    });
...
function verifyDataBeforeSubmit() {
    ...
    swal("Error!", "กรุณาระบุกำหนดส่ง", "error");
    $('#dispatch').focus();
    return false;
}
...
function onSubmit() {
    if (verifyDataBeforeSubmit()) {
        ...
        $.ajax({
            ...
            if (state === 'OKAY') {
                swal("Completed!", "เพิ่มใบสั่งงานเข้าฐานข้อมูลเรียบร้อยแล้ว "+$('#workorderId').val(), "success");
                onDupicate(true);
            } else if (state === 'UPDATED') {
                swal("Completed!", "ปรับปรุงใบสั่งงานเรียบร้อยแล้ว "+$('#workorderId').val(), "success");
                onDupicate(true);
...
function onSubmitDuplicate() {
    swal({
        title: "คุณต้องการทำซ้ำใบสั่งงานนี้หรือไม่",
        icon: "info",
        dangerMode: true,
        buttons: true
    }).then(
